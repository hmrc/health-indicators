...
class LeakDetectionMetricProducerSpec
  extends AnyWordSpec
     with Matchers
     with MockitoSugar
  ...
  "LeakDetectionMetricProducer.produce" should {
    "Return a Metric with no results when leak detection connector returns None" in {
      when(mockLeakDetectionConnector.findLeaks(eqTo("foo"))(any[HeaderCarrier]))
        .thenReturn(Future.successful(Nil))

      val result = producer.produce("foo")

      result.futureValue shouldBe Metric(LeakDetectionMetricType, Seq(Result(LeakDetectionNotFound, "No Leaks Detected", None)))
    ...
    "Return a Metric with 2 results when Report with 2 violations is found" in {
      ...
      result.results.count(_.resultType == LeakDetectionViolation) shouldBe 2
    }
    ...
    "Cap the total number of leaks reported" in {
      ...
      result.results.count(_.resultType == LeakDetectionViolation) shouldBe producer.maxLeaks + 1
      result.results.last.description shouldBe s"Has ${lotsOfLeaks.length-producer.maxLeaks} additional unresolved leaks."
    }
  }
}
